...
////Anja's additions
// small chromosmes were not passed to the final phased panel output, this fixes that
include { STAGE_PANEL_PHASED } from '../../modules/local/stage_panel_phased'
...
workflow PHASEIMPUTE {
    ...
    if (params.steps.split(',').contains("panelprep") || params.steps.split(',').contains("all")) {
        ...
        if (params.phase == true) {
            ...
            // Explicitly stage all phased panel VCF+CSI files to prep_panel/panel
            STAGE_PANEL_PHASED(VCF_PHASE_SHAPEIT5.out.vcf_tbi)

            // Use the staged output as the canonical phased panel channel downstream
            ch_panel_phased = STAGE_PANEL_PHASED.out.vcf_tbi

            ch_versions = ch_versions.mix(VCF_PHASE_SHAPEIT5.out.versions)
            ch_versions = ch_versions.mix(STAGE_PANEL_PHASED.out.versions)
        }
    ...
    // Get all target VCF files from input CSV (only for VCF format inputs)
    ch_target_vcf = Channel.empty()
    if (params.input) {
        // Check if input is VCF format (has 'file' or 'vcf' column)
        def headers = file(params.input).readLines()[0].split(',')*.trim()*.toLowerCase()
        if (headers.contains('file') || headers.contains('vcf')) {
        ch_target_vcf = Channel
            ...
            // PLINK or Mixblup input - VCF will be generated by conversion
    ...
    // Get imputed VCF files (from concatenated outputs, before per-sample split)
    // ch_imputed_concat contains [meta, vcf, tbi] tuples from CONCAT_* steps
    ch_imputed_vcf_stats = ch_imputed_concat
